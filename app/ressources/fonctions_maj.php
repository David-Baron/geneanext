...
function OK_taille_img_2plans($hauteur, $largeur)
{
    ...
    if ($hauteur > $largeur) {
        if (($hauteur > $taille_maxi_images['w']) or
            ($largeur > $taille_maxi_images['h'])
        ) $ctrl_OK = false;
    }
    // Mode paysage
    else {
        if (($hauteur > $taille_maxi_images['h']) or
            ($largeur > $taille_maxi_images['w'])
        ) $ctrl_OK = false;
    }
    return $ctrl_OK;
}
    ...
    if ($err_fic) {
        switch ($err_fic) {
            case 1: // UPLOAD_ERR_INI_SIZE
                $message = 'Le fichier excède le poids autorisé (' . $taille_maxi . ')';
                break;
            case 2: // UPLOAD_ERR_FORM_SIZE
                $message = 'Le fichier excède le poids autorisé par le champ MAX_FILE_SIZE s\'il a été donné';
                break;
